...
import { Card, Grid, HStack, Box, Text, Skeleton } from "@chakra-ui/react";
    ...
    <HStack w="full" px={8} spacing={0}>
      <Card flex={1} minW="fit-content" variant="outline">
        <Skeleton isLoaded={!!entries}>
          ...
          >
            <TableHeader />
            {entries?.entries.map((entry) => <EvaluationRow key={entry.id} entry={entry} />)}
