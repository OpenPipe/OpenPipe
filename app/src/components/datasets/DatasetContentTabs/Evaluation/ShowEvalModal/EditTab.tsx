import { Button, Checkbox, HStack, Input, VStack, Text } from "@chakra-ui/react";
...
export const EditTab = () => {
  ...
  const [onSaveConfirm, saveInProgress] = useHandledAsyncCallback(async () => {
    ...
    ensureEvalShown(showEvalModalId);
    addFilter({
      id: Date.now().toString(),
      field: EvaluationFiltersDefaultFields.EvalApplied,
      comparator: "=",
      value: showEvalModalId,
    });

    setShowEvalModalId(null);
    ...
    <VStack alignItems="flex-start" w="full" spacing={8}>
      ...
      <VStack alignItems="flex-start" w="full">
        ...
        <Input
          ...
          type="number"
          onChange={(e) => setNumDatasetEntries(parseInt(e.target.value.match(/\d+/g)?.[0] ?? ""))}
          placeholder="100"
          w="full"
        />
        <Text bgColor="orange.50" borderColor="orange.500" borderRadius={4} borderWidth={1} p={2}>
          These changes will add <b>{numAddedComparisons}</b> head-to-head comparisons and cost
          approximately <b>${(numAddedComparisons * 0.06).toFixed(2)}</b>.
        </Text>
      </VStack>
      <VStack alignItems="flex-start" w="full">
