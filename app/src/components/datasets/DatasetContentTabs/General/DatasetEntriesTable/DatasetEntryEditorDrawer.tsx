...
function DatasetEntryEditorDrawer({
  ...
  const drawerIsOpen = !!datasetEntryId;
  const [entryInitiallyLoaded, setEntryInitiallyLoaded] = useState(false);
  useEffect(() => {
    if (!drawerIsOpen) setEntryInitiallyLoaded(false);
    if (datasetEntry?.persistentId) setEntryInitiallyLoaded(true);
  }, [drawerIsOpen, datasetEntry?.persistentId]);
