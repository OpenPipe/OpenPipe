...
const DeleteFineTuneDialog = ({
  ...
  const formattedSlug = formatFTSlug(fineTuneSlug);
        ...
        <AlertDialogContent>
          ...
          <AlertDialogBody>
            <VStack spacing={4} alignItems="flex-start">
              <Text>
                If you delete this fine-tuned model any future requests to <b>{formattedSlug}</b>{" "}
                will fail. If you are sure that you want to delete this model, please type its ID
                below. This action cannot be undone.
              </Text>
              <Box bgColor="orange.100" w="full" p={2} borderRadius={4}>
                <Text fontFamily="inconsolata">{formattedSlug}</Text>
              </Box>
              <Input
                placeholder={formattedSlug}
                value={slugToDelete}
          ...
          <AlertDialogFooter>
            <HStack>
              ...
              <ConditionallyEnable
                ...
                checks={[[slugToDelete === formattedSlug, "Enter the correct ID"]]}
              >
