...
The simplest way to start ingesting request logs into OpenPipe is by installing our drop-in Python or TypeScript SDK. Requests to both OpenAI and OpenPipe models will automatically be logged.
Logging doesn't add any latency to your requests, because the SDK calls the OpenAI server directly and returns your completion before kicking off a request to record it in your project.
...
```python
{
    ...other headers,
    "op-log-request": "true",
    # Add searchable tags is highly recommended
    "op-tags": '{"prompt_id": "first_prompt"}',
}
```

Here's an example of steps 2-4 put together in both a raw cURL request and the Python SDK:
